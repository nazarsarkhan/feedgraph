...
// classifyError is private; we cast through `unknown` so the intersection
// doesn't reduce to `never` against the private member. Exposing it as
// protected/public just for testability would leak an implementation
// detail into the public surface.
...
describe('OpenAiAdapter.classifyError', () => {
  ...
  beforeEach(() => {
    adapter = new OpenAiAdapter('fake-key', 'gpt-4o-mini') as unknown as AdapterWithPrivate;
  });
  ...
  it('classifies 400 as NON-retriable (request-shape fault repeats on failover)', () => {
