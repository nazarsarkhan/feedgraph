...
- [ ] **Auth route gating not yet implemented.** Every route (`/articles`, `/entities`, `/feeds`, `/graph`, `/settings`) is publicly accessible until the next milestone wires `useQuery(['me'])` and a `<ProtectedRoute>` wrapper. Until then the pages render as placeholders without authentication checks.
