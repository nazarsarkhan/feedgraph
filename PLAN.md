...
- [ ] **Toast notifications for success states.** "Confirmation email resent" and the future "feed added" / "settings saved" affordances are currently inline messages. A toast system (e.g. shadcn `sonner` add-on) would unblock those plus give us a one-place sink for transient confirmations and non-fatal errors.
