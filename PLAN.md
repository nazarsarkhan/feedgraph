...
- [ ] **Extract `useUrlListFilters<TFilters>()` + generic `ListPage` when a third filter-driven list appears.** `useArticleFilters` and `useEntityFilters` share the same structural shape (URL state, `setFilter` with page-reset, `reset`, `activeFilterCount`). Two examples isn't enough to design against, but the moment a third lands (graph node-list, settings table, etc.) the duplication becomes design-by-instance. Trigger explicit; not a free-standing refactor.
- [ ] **`useArticleFilters` should grow a batch `setFilters(Partial<…>)` too.** The articles page's sort dropdown calls `setFilter('sortBy', x); setFilter('order', y)` synchronously and is latently buggy for the same React Router reason that drove `setFilters` into `useEntityFilters`. Currently masked because `publishedAt:desc` is the default and most users don't toggle to a non-default direction. Fix by lifting the `setFilters` helper into a shared utility (or directly into the eventual `useUrlListFilters`) and migrating both pages.
- [ ] **`minMentions` quick-toggle UI (1+ / 5+ / 10+) instead of a free number input.** The current free-form number input requires the user to type a value with no anchor. A trio of toggle buttons (`1+`, `5+`, `10+`) plus a "custom" entry would let the common case be one click. Skipped for now because we don't have a sense of which thresholds matter in practice — wait for usage data.
- [ ] **Semantic color hints per entity type (person / company / product / technology / location) on type badges.** All five types render as outline badges today, which keeps the row visually flat and predictable but loses scannability at scale. When entity counts grow into the hundreds, color-coding the type badge (e.g. a hue per type token) becomes worth the perceived complexity. Deferred until the typical list view holds enough entities to benefit.
